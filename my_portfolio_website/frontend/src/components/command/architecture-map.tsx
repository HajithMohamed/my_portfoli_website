...
export function ArchitectureMap() {
  ...
  // Keyboard navigation between architecture nodes
  const handleKeyDown = (e: KeyboardEvent<SVGGElement>, index: number) => {
    let nextIndex = index;
    if (e.key === "ArrowRight" || e.key === "ArrowDown") {
      ...
      e.preventDefault();
      setHover(hover === NODES[index].id ? null : NODES[index].id);
      return;
    ...
    <Panel label="architecture.map" subtitle="production stack" live className="h-full">
      <div className="relative w-full overflow-x-auto touch-pan-x hide-scrollbar rounded-md border border-cyan/10 bg-[#02050a]">
        ...
        <svg
          ...
          role="region"
          aria-label="Interactive architecture diagram of Mohamed Hajith's portfolio"
        >
          <defs>
          ...
          {/* Edges with stroke-dashoffset animation on first scroll into view */}
          {EDGES.map(([a, b], i) => {
            const A = posById[a];
            const B = posById[b];
            if (!A || !B) return null;
            ...
            return (
              <g key={i}>
                <motion.line
                  ...
                  transition={{ duration: prefersReduced ? 0 : 1.2, delay: prefersReduced ? 0 : i * 0.08, ease: "easeInOut" }}
                />
          ...
          {NODES.map((n, i) => {
            ...
            return (
              <motion.g
                ...
                tabIndex={0}
                role="button"
                aria-label={`Node ${n.label} in ${n.type} tier. Press Enter to trace.`}
                onMouseEnter={() => {
                  setHover(n.id);
                  setFocusedIndex(i);
                }}
                ...
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: prefersReduced ? 0 : 0.4, delay: prefersReduced ? 0 : 0.2 + i * 0.04 }}
              >
                {/* Node halo */}
                {(active || isFocused) && (
                  <circle
                ...
                <circle
                  cx={n.x}
                  cy={n.y}
                  r={active || isFocused ? 3.5 : 2.5}
                  fill="none"
                  stroke={color}
                  strokeOpacity={active || isFocused ? 0.9 : 0.3}
                  strokeWidth={isFocused ? 0.35 : 0.2}
                  strokeDasharray={isFocused ? "0.6 0.6" : "none"}
                />
                ...
                <circle
                  cx={n.x}
                  cy={n.y}
                  r={active || isFocused ? 2 : 1.5}
                  fill="#02050a"
                  stroke={color}
                  strokeWidth={active || isFocused ? 0.5 : 0.3}
                />
                ...
                <text
                  ...
                  y={n.y - 4.5}
                  textAnchor="middle"
                  fill={active || isFocused ? "#fff" : "rgba(226,236,255,0.7)"}
                  fontSize="2"
                  fontFamily="monospace"
                  fontWeight={active || isFocused ? "bold" : "normal"}
                  style={{ textShadow: active || isFocused ? `0 0 5px ${color}` : "none" }}
                >
                  {n.label}
                </text>
              </motion.g>
      ...
      <div className="mt-4 font-mono text-[11px] text-muted-foreground flex items-start gap-2">
        ...
        <p>
          <span className="text-cyan/90 font-medium">Trace connections:</span> Tap/Hover a node or use{" "}
          <kbd className="rounded border border-cyan/30 px-1 py-0.5 text-[9px] text-cyan">Arrow keys</kbd>{" "}
          and <kbd className="rounded border border-cyan/30 px-1 py-0.5 text-[9px] text-cyan">Enter</kbd> to trace.
          This illustrates the verified production architecture of this platform.
        </p>
      </div>
    </Panel>
  );
